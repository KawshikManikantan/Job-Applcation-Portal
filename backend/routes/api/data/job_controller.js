...
router.post("/all",auth,
    async (req, res) => {
        ...
        try {
            ...
            const num_applications= await Application.countDocuments({applicant:req.body.applicantid,status:{$in:['Applied','Shortlisted']}})
            ...
            const existingJobs = await Job.find({deadline:{$gte:Date.now()}}).lean().populate('recruiter')
            if(existingJobs.length!==0)
            {
                ...
                const newsent=tobesent.filter((job,ind)=>{
                    if(job.num_selected!==job.max_positions)
                        return job
                })
                console.log(newsent)
    ...
    async (req, res) => {
        ...
        try {
            ...
            if(existingJobs.length!==0)
            {
                ...
                return res.status(200).json(newsent)
            }
...
router.post("/delete",
    async (req, res) => {
        ...
        try {
            ...
            await Application.deleteMany({job: req.body.job_id})
