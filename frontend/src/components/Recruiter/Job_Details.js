...
const schema = yup.object().shape({
    ...
    max_positions:yup.number().positive().required(),
    max_applications:yup.number().min(1).required()
                        .moreThan(
                        yup.ref('max_positions'),
                        'Should be more than max_positions'),
    // deadline:yup.date().required().min(new Date()),
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="form-group">
                    <input type="submit" value={props.location.state?"Update":"Register"} className="btn btn-primary"/>
                </div>
            </form>
        </div>
    )
