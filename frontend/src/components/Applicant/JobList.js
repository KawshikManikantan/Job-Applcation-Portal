...
class JobList extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {jobs: [],userstatus: [],expandstate:[],user:'',sortby:['duration','salary','rating'],sortstate:[2,2,2],renderjobs:[],searchtab:'',typefilter:'None',minsal:'',maxsal:'',maxdur:'None'};
        var ini=[]
    ...
    handleSearch(){
            ...
            if(this.state.searchtab!=='')
            {
                ...
                update=arr.map((el,ind)=>el.item)
                console.log('Fuse')
                console.log(update)
            }

            if(this.state.minsal!=='' && this.state.maxsal!=='' && !(isNaN(parseInt(this.state.maxsal))) && !(isNaN(parseInt(this.state.minsal))) && parseInt(this.state.minsal)>=0 && parseInt(this.state.maxsal)>=0 && (parseInt(this.state.minsal)<=parseInt(this.state.maxsal)))
            {
                console.log("Enters check")
                update=update.filter((upd,ind)=>{
                    if(upd.salary>=parseInt(this.state.minsal) && upd.salary<=parseInt(this.state.maxsal))
                    {
                        return(upd)
                    }
                })
            ...
            console.log(update)
            //
            if(this.state.maxdur!=='None')
            {
                update=update.filter((upd,ind)=>{
                    if(upd.duration<parseInt(this.state.maxdur))
                    {
                        return(upd)
                    }
                })
            ...
            {
                update=update.filter((upd,ind)=>{
                    if(upd.type===this.state.typefilter)
                    {
                        return(upd)
                    }
                })
