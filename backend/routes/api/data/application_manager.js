...
const ApplicantRating=require("../../../models/Applicant_Rating")
        ...
        try {
           ...
           if(req_details.length===0)
           {
               ...
               const tobesent = await Promise.all(req_details.map(async (det) => {
                   const rating_details = await ApplicantRating.findOne({
                       job: det.job._id,
                       applicant: det.applicant._id
                   }).lean()
                   if (!rating_details) {
                       return ({
                           ...det,
...
router.post("/recruiter/job",auth,
    async (req, res) => {
        ...
        console.log(req.body)
        try {
            const req_details=await Application.find({job: req.body.job_id,status:{$ne : "Rejected"}}).lean().populate([{path:'job',model:'Job',populate: {
                    path: 'recruiter',
                    model: 'Job_Recruiter',
                }},'applicant'])
            return res.status(200).json(req_details)
    ...
    async (req, res) => {
        ...
        try {
            ...
            if(req.body.status==='Accepted'){
                const rejectall_req=await Application.updateMany({applicant:req.body.applicant,status:{$ne:'Accepted'}},{ $set: { status: 'Rejected' } })
