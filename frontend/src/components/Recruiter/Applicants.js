...
class Rec_Applicants extends Component {
    ...
    async handleMod(value,ind){
        try {
            ...
            if(value==='Accepted')
            {
                const templateId = 'template_99755zf';
                const serviceID = 'jobsearch';
                console.log(this.state.renderapplications[ind].applicant.name)
                console.log(this.state.renderapplications[ind].job.title)
                console.log(this.state.renderapplications[ind].job.recruiter.name)
                await emailjs.send(serviceID, templateId, {email:this.state.renderapplications[ind].applicant.email,to_name:this.state.renderapplications[ind].applicant.name,post:this.state.renderapplications[ind].job.title,recruiter:this.state.renderapplications[ind].job.recruiter.name},'user_8NWli6K2sbsRjVVEkYZqk')
                const tosend={
